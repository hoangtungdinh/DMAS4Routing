 ...
 *
 */
public class FreeIntervalFinder {

  /**
   * Finds gaps on the time line between a list of existing {@link Interval} and
   * a search {@link Interval}
   * 
   * @param existingIntervals
   * @param searchInterval
   * @return The list of gaps
   */
  public List<Interval> findGaps(List<Interval> existingIntervals,
    ...
    // searchInterval
    List<Interval> subExistingList = removeNoneOverlappingIntervals(
        existingIntervals, searchInterval);
    DateTime subEarliestStart = subExistingList.get(0).getStart();
    DateTime subLatestStop = subExistingList.get(subExistingList.size() - 1)
        .getEnd();
  ...
  private List<Interval> removeNoneOverlappingIntervals(
      List<Interval> existingIntervals, Interval searchInterval) {
  ...
  private boolean hasNoOverlap(List<Interval> existingIntervals,
      Interval searchInterval, DateTime searchStart, DateTime searchEnd) {
    DateTime earliestStart = existingIntervals.get(0).getStart();
    DateTime latestStop = existingIntervals.get(existingIntervals.size() - 1)
        .getEnd();
